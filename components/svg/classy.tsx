import { SvgIcon } from "@mui/material";

const ClassySVG = () => (
  <SvgIcon>
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlSpace="preserve"
      style={{
        fillRule: "evenodd",
        clipRule: "evenodd",
        strokeLinejoin: "round",
        strokeMiterlimit: 2,
      }}
      viewBox="0 0 100 100"
    >
      <circle cx={43} cy={31} r={19} transform="translate(-21 11)" />
      <circle cx={43} cy={31} r={19} transform="translate(34 -10)" />
      <circle cx={43} cy={31} r={19} transform="translate(-21 46)" />
      <circle cx={43} cy={31} r={19} transform="translate(15 46)" />
      <path d="M41 58V45.702C41 33.164 30.836 23 18.298 23H3v73h74V74.155A16.155 16.155 0 0 0 60.845 58H41ZM96 2H58v19.875A18.124 18.124 0 0 0 76.125 40H96V2Z" />
    </svg>
  </SvgIcon>
);
export default ClassySVG;
